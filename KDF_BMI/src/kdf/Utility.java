...
public class Utility {
	...
	public void browser_open(String fURL) {
		if (webDriver == null) {
			System.setProperty("webdriver.chrome.driver", "D:/chromedriver91.exe");
	...
	public String actual_result() {
		String actualresult = "";
		try {
			actualresult = webDriver.findElement(By.xpath("(//div[@class='rightresult']//b)[1]")).getText()
					.replaceAll("BMI = ", "").replaceAll(" kg/m2", "");
		} catch (Exception e) {
			actualresult = webDriver.findElement(By.xpath("//div[@class='rightresult']//font")).getText();
		}
		return actualresult;
	}
	...
	public void saveScreen(int index) {
		...
		try {
			FileUtils.copyFile(scrFile, new File("D:\\eclipse-workspace-tester\\KDF_BMI\\x" + index + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
