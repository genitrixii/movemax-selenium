...
public class StartUp {
	public static String vUrl, vCompanyTh, vCompanyEn, vCompanyAc, vTaxBranchNo, vTaxIdNo, vAddress, vAddress2,
			vOfficeTel, vTel, vFax, vEmail, vLine, vFacebook, vWebsite, vContact, vAlert;
	...
	public void main() throws Exception {
		...
		String xlPath_tc = "D:/eclipse-workspace-tester/KDF_MoveMax/Excel/TCResult/TC001_Result.xlsx";
		String xlPath_ts = "D:/eclipse-workspace-tester/KDF_MoveMax/Excel/TSResult/TS001_Result.xlsx";
		String xlPath_td = "D:/eclipse-workspace-tester/KDF_MoveMax/Excel/TDResult/TD001_Result.xlsx";
		String xlPath = "D:/eclipse-workspace-tester/KDF_MoveMax/Excel/TC001_AddCompany.xlsx";
						...
						for (int j = 1; j < xTSdata.length; j++) {
							if (xTCdata[i][1].equals(xTSdata[j][0])) {
								...
								if (!vError.equals("No Error")) {
									...
									xTDdata[k][21] = vResult;
									xTDdata[k][22] = vError;
								}
								xTSdata[j][8] = vResult;
							}
							if (xTDdata[k][20].equals(xTDdata[k][19])) {
								vResult = "Pass";
								StartUp.xTDdata[k][22] = StartUp.vError;
							} else {
								vResult = "Fail";
								StartUp.xTDdata[k][22] = "Mismatch alert";
							}
							StartUp.xTDdata[k][21] = vResult;
						}
						System.out.println(StartUp.xTDdata[k][0]);
						System.out.println("Expected = " + StartUp.xTDdata[k][19]);
						System.out.println("Actual = " + StartUp.xTDdata[k][20]);
						System.out.println("Result = " + StartUp.xTDdata[k][21]);
					}
					if (xTDdata[k][21] == "Fail") {
						fails += 1;
					} else if (xTDdata[k][21] == "Pass") {
						passes += 1;
					}
				}
				if (fails != 0) {
